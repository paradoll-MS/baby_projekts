...
from scent_classifier import (classify_product, KEYWORD_TO_FAMILY, FAMILIES,
                              _classify_one_sync, _classify_text,
                              detect_unscented, cleanup_playwright)
...
st.success(f"✅ 完成！已分类 {ok_count - unscented} 条，无香 {unscented} 条，共 {max_n} 条")

# 调试面板：查看原始分类结果
with st.expander("🔍 查看原始分类结果"):
    for i, r in enumerate(results):
        if r:
            m = r.get("方法", "?")
            tag = "⚡cffi" if m == "cffi" else "🐢pw" if m == "pw" else "📋local" if m == "local" else f"❓{m}"
            sub = r.get("细分_clean", "") or r.get("细分", "")
            st.text(f"[{i+1}] [{tag}] {r['大类'][:40]} | {sub[:60]} | {products[i][:60]}")
